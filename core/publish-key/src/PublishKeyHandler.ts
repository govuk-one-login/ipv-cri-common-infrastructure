...
export const logger = new Logger({ serviceName: "PublishKeyHandler", logLevel: "INFO" });
...
export class PublishKeyHandler implements LambdaInterface {
  ...
  constructor(decryptionKeyID: string, bucketName: string) {
	...
	async getAsJwk(keyId: string): Promise<Jwk | null> {
		...
		try {
			...
			console.log("successfully obtained kmsKey = " + kmsKey);
		} catch (error) {
