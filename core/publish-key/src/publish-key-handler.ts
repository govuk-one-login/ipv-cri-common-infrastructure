...
const POWERTOOLS_LOG_LEVEL = process.env.POWERTOOLS_LOG_LEVEL ?? "INFO";
const logger = new Logger({ serviceName: "PublishKeyHandler", logLevel: POWERTOOLS_LOG_LEVEL });
...
export class PublishKeyHandler implements LambdaInterface {
    ...
    public async handler(): Promise<string> {
		...
		if (!ENCRYPTION_KEY_IDS || !JWKS_BUCKET_NAME) {
			logger.error({ message:"Environment variable ENCRYPTION_KEY_IDS or JWKS_BUCKET_NAME is not configured" });
